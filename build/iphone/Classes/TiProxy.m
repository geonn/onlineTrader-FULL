...
//Rare exceptions to indicate a bug in the _onlinetrader code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_onlinetrader = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_onlinetrader]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.onlinetrader.module"
