...
NSString * OnlineTrader$ModuleRequireFormat = @"(function(exports){"
...
@implementation OnlineTraderObject
...
{
	// create OnlineTrader global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "OnlineTrader" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_onlinetrader = [[OnlineTraderObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_onlinetrader];
	
	NSString *_onlinetraderNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _onlinetraderNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_onlinetrader valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:OnlineTrader$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.onlinetrader.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.onlinetrader.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
